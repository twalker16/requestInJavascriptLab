    ...
    axios.get('http://localhost:3000/animals')
        .then(res => {
            ...
            for(let i = 0; i < res.data.length; i++){
                ...
                newAnimal.textContent = res.data[i]
                document.body.append(newAnimal)
            }
        })
...
function queryRequest(){
    axios.get('http://localhost:3000/query-test/?animals=bats&zoo=clean')
        .then(res =>{
            console.log(res.data)
        })
        .catch(error =>console.log(error))
